...
const listBuckets = async () => {
  try {
    ...
    console.error(
      "This might be a permission issue. Please ensure your R2 API token has 'List All Buckets' permission."
    );
    throw error;
  }
};
...
const migrateImages = async () => {
  try {
    ...
    if (!availableBuckets.includes(BUCKET_NAME)) {
      ...
      console.error(
        `Please check the R2_BUCKET_NAME in your .env file and make sure it matches one of the available buckets listed above.`
      );
      console.error(`---`);
